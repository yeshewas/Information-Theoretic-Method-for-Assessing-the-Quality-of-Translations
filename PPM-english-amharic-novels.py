...
def compress_with_ppmd(text, temp_file="temp_ppmd.txt", compressed_file="temp_ppmd.7z"):
    try:
        with open(temp_file, "w", encoding="utf-8") as f:
            f.write(text)

        subprocess.run(
            [SEVEN_ZIP_PATH, "a", "-m0=PPMd", compressed_file, temp_file],
        ...
        print(f"Error in PPMd compression: {e}")
        return float('inf')
def calculate_compression_difference(training_data, test_slice):
    ...
    combined_size = compress_with_ppmd(combined_data)
    training_size = compress_with_ppmd(training_data)
...
if __name__ == "__main__":
    ...
    # Load training data (128 kB per author)
    training_data_map = {
        author: load_text(file_path, 128) for author, file_path in training_files.items()
    }

    # Load test slices (32 slices per file, 4 kB each)
    test_slices = {
